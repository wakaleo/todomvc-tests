...
This method takes a single parameter which represents the name of the actor ("Trudy" in tis case). In future versions of the tests we might need to cater for different actors, but for now we can ignore the `actor` parameter for now. All we need to do is to open the TodoMVC page. We do this by calling the `toTheTodoMVCApplication()` method in the `NavigateActions` class.

Let's look at the `NavigateActions` class.
